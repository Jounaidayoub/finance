...
flowchart TD
    ...
    %% X e1@==> Y
    A[data source] e3@==>
    Kafka_broker["kafka broker"]--produce-->B(Kafka.consumer.: financial_prices);
     
    %% B -- Price Stream -->C@{ shape: processes, label: "Multiple workers" }
    B -- Read from Topic -->Redis[("Redis Task queue")];
    C -- Anomaly? --> D[🔔 Alerting Log/API] 
    D e2@==> alerts
    alerts[alerts topic];
    E[(" Elasticsearch")];
    C e5@==Anomalis & Row data==> E
    e5@{ animate: true }
    e2@{ animate: true }
    ...
    G e7@ == 🔎 Queries ==> E;
    e7@{ animate: true }
    G -- Generates --> H[📆 Daily/Monthly... Reports];
    I[FastAPI] e6@<-- " 🔍 Queries" --> E;
    e6@{ animate: true }    
    J[👥 User/Client] --> I;
    e3@{ animate: true }
    ...
    %%     B; C; D;
    %% end
    subgraph celery and Redis
        %% direction TB

        C@{ shape: processes, label: " ⚒ Multiple workers" }
